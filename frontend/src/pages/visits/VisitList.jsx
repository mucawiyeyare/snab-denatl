  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={`Request Laboratory Tests — ${selectedVisit?.patient_id?.name || 'Patient'}`}
        maxWidth="max-w-lg"
      >
        <form onSubmit={submitLabRequest} className="space-y-4 text-xs">
          
          {/* One-Time Testing Fee Notice */}
          <div className="p-3 bg-purple-50 border border-purple-200 rounded-2xl flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-purple-700">Fixed rate applied once for all selected tests in this session.</p>
              </div>
            </div>
            <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-purple-200/80 text-purple-900">
              {labRequestForm.test_ids?.length || 0} Selected
            </span>
          </div>
          ...
          <div>
            ...
            {/* Test Selection list with clear toggles */}
            <div className="grid grid-cols-1 gap-2 max-h-56 overflow-y-auto p-2 bg-slate-50 rounded-2xl border border-slate-200">
              {labTests.map(t => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="font-bold text-xs">{t.test_name}</span>
                    </div>
                    <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded-md text-slate-600 font-medium">{t.category}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div>
            <label className="block font-bold text-slate-700 mb-1">Clinical Reason / Doctor Indication</label>
            <input
              type="text"
              ...
              className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-purple-500"
            />
          </div>
          ...
          <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
            ...
            <button
              ...
              className="px-5 py-2 bg-purple-600 hover:bg-purple-700 active:scale-95 text-white rounded-xl font-bold shadow-md shadow-purple-500/20 disabled:opacity-50 flex items-center gap-1.5 transition cursor-pointer"
            >
      ...
      <Modal
        ...
        title={`Laboratory Results — ${viewingLabResultData?.visit?.patient_id?.name || 'Patient'}`}
        maxWidth="max-w-xl"
      >
        <div className="space-y-4 text-xs">
          ...
          <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-1">
            ...
            <div className="flex justify-between items-center pt-1 border-t border-slate-200/60">
              <span className="text-slate-500 font-medium">Testing Fee:</span>
              <span className="font-mono font-bold text-emerald-700">$3.00 (One-time unified charge)</span>
            </div>
          </div>

          <div className="space-y-3">
            <h4 className="font-bold text-slate-800 text-xs uppercase tracking-wider text-slate-500">Laboratory Outcomes:</h4>
            
            {(viewingLabResultData?.results || []).length === 0 && (viewingLabResultData?.requests || []).length === 0 ? (
              <div className="text-center py-8 p-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                <p className="text-slate-500 font-semibold mb-1">No completed lab results recorded yet.</p>
                <p className="text-[11px] text-slate-400">If the patient was just sent to the lab, results will appear here once entered by the cashier/lab staff.</p>
              </div>
            ) : (
              (viewingLabResultData?.requests || []).map((req, idx) => {
                const reqResults = (viewingLabResultData?.results || []).filter(
                  r => r.request_id?.toString() === req._id?.toString() || r.request_id?._id?.toString() === req._id?.toString()
                );

                const hasTestsArray = req.tests && req.tests.length > 0;
                const displayTests = hasTestsArray
                  ? req.tests
                  : (reqResults.length > 0
                    ? reqResults.map(r => ({
                    ...
                    : [{ test_name: req.test_name || 'Laboratory Test', result: req.result || 'Pending' }]
                  );

                return (
                  <div key={idx} className="p-4 border border-purple-200 bg-purple-50/40 rounded-2xl space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] font-bold text-purple-700 uppercase tracking-wider block font-mono">
                          Request #{req.request_number || `REQ-${idx + 1}`}
                        </span>
                        <span className="font-black text-slate-900 text-sm block mt-0.5">
                          {displayTests.length} Laboratory Tests
                        </span>
                        <span className="text-[10px] text-slate-500">Indication: {req.reason || 'Pre-treatment screening'}</span>
                      </div>
                      <div className="text-right">
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-200">
                          {req.payment_status === 'Paid' ? 'Fee Paid ($3.00)' : 'Fee: $3.00'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      {displayTests.map((t, tIdx) => {
                        const resStr = t.result || 'Pending';
                        const isPos = resStr.toLowerCase().includes('positive') || resStr.toLowerCase().includes('reactive');
                        const isNeg = resStr.toLowerCase().includes('negative') || resStr.toLowerCase().includes('non-reactive');

                        return (
                          <div key={tIdx} className="p-3 bg-white border border-slate-200 rounded-xl flex items-center justify-between gap-3 shadow-2xs">
                            <div>
                              <span className="font-bold text-slate-900 text-xs block">{t.test_name}</span>
                              {t.reference_range && (
                                <span className="text-[10px] text-slate-400 block">Ref: {t.reference_range}</span>
                              )}
                            </div>
                            <div className="flex items-center gap-2">
                              <span className={`px-3 py-1 rounded-full text-xs font-black tracking-wide ${
                                ...
                                resStr === 'Pending' ? 'bg-amber-100 text-amber-800 border border-amber-200' :
                    ...
                    <div className="flex justify-between text-[10px] text-slate-500 pt-1 border-t border-purple-100">
                      <span>Entered by: <strong>{req.performed_by || 'Lab Staff'}</strong></span>
                      <span>{new Date(req.completed_date || req.updatedAt).toLocaleDateString()}</span>
                    </div>

                    {req.notes && (
                      <p className="text-[11px] text-slate-600 bg-white/90 p-2.5 rounded-xl border border-slate-100">
                        <strong>Clinical Notes:</strong> {req.notes}
                      </p>
                    )}
                  </div>
                );
          ...
          <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
            ...
            <button
              ...
              className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white rounded-xl font-bold shadow-md shadow-emerald-500/20 flex items-center gap-1.5 transition cursor-pointer"
            >
