...
const BillingManager = () => {
  ...
  const fetchInvoices = async () => {
    setLoading(true);
    try {
      // Fetch ALL invoices at all times
  ...
  const fetchTodayIncome = async () => {
    try {
      ...
      const res = await getDailyIncomeApi({ date: todayStr });
      if (res.data?.data?.summary?.total_income !== undefined) {
        setTodayCollected(Number(res.data.data.summary.total_income) || 0);
      }
    } catch (e) {
      console.error('Error fetching today income:', e);
    }
  };
  ...
  // Accurate aggregate totals for all invoices
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `SNAB_All_Invoices_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── 3 Summary KPI Cards — ALL INVOICES + TODAY'S COLLECTED INCOME ── */}
